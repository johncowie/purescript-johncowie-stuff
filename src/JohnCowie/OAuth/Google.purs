...
redirect config = config.oauthUrl <> query
  where query = queryString $
                ...
                , Tuple "redirect_uri" config.callbackUrl
                ]

fetchOpenIdData :: GoogleConfig -> OAuthCode -> Aff (Either String {access_token :: JWT, id_token :: JWT})
fetchOpenIdData config code = runExceptT $ do
...
-}
handleCode :: GoogleConfig -> OAuthCode -> Aff (Either String UserData)
handleCode config code = runExceptT do
  tokenData <- ExceptT $ fetchOpenIdData config code
...
oauth = mkComponent
  ...
  , callbackUrl: var "GOOGLE_CALLBACK_URL" # describe "Url for google to return to on oauth completion"}
